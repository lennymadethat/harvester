// memory.js — read and write pages in a Second Brain library (Postgres + pgvector,
...
// ---- Harvester additions: read a lens's rulebook pages live from the library ----

// Every live page under a prefix (e.g. "wiki/methodology/content-rules/"), path + body.
export async function listPagesByPrefix(prefix, env) {
  const url = new URL(`${env.SUPABASE_URL}/rest/v1/pages`);
  url.searchParams.set('path', `like.${prefix.replace(/%/g, '')}*`);
  url.searchParams.set('deleted_at', 'is.null');
  url.searchParams.set('select', 'path,body');
  url.searchParams.set('order', 'path.asc');
  const resp = await fetch(url.toString(), { headers: headers(env) });
  if (!resp.ok) throw new Error(`library prefix list ${resp.status}: ${(await resp.text()).slice(0, 200)}`);
  return resp.json();
}

// Several pages by exact path; a missing page is skipped, never fatal.
export async function readManyPages(paths, env) {
  const out = [];
  for (const p of paths) {
    try { const page = await readPage(p, env); if (page && typeof page.body === 'string') out.push({ path: page.path, body: page.body }); }
    catch { /* a lens with a missing file still synthesizes, with less context */ }
  }
  return out;
}
